...
public class MFD01B004Z00 extends MAA00B006Z00 {
	...
	public void execute() throws Exception {
		...
		//処理結果Map In Map <Key:患者ID　value:項目Map<項目 名前,値>> ※Keyで自然ソート。
		...
		//ADOより診療記録TBLを取得
		List<MDB03T001Z00> snRecoTbls = super.getAllInOneData().getSnRecoTbls();
		...
		for (MDB03T001Z00 snRecoDto : snRecoTbls) {
			...
			if (resultMap.containsKey(targetId) && !prcssdList.contains(targetId)) {
				...
				if (targetSrt.compareTo(fDateDate) <= 0
					...
					String title = super.getAllInOneData().getAppData().getMsgIn(MAAT00.ITM_NME_1);
					...
					if (subject.contains(title)) {
						...
						for (int i = 0; i < ITM_NME_LIST.length; i++) {
							...
							//項目１　項目名　取得
							String nme1 = super.getAllInOneData().getAppData().getMsgIn(ITM_NME_LIST[i]);
							...
							if (!nme1.equals(MAAT00.CHAR.EMPTY_STRING)
								...
								//項目１　区切り文字　取得
								String dlm1 = super.getAllInOneData().getAppData().getMsgIn(ITM_DLM_LIST[i]);
								//項目１　終了文字　取得
								String end1 = super.getAllInOneData().getAppData().getMsgIn(ITM_END_LIST[i]);
								//終了文字のインデックスを定義
								int indexEnd1 = 0;
...
//								System.out.println("格納文字⇒" + targetId + "：" + nme1 + "：" + result1.replaceFirst("^[\\h]+", "").replaceFirst("[\\h]+$", ""));
		...
		logOut.setResultMap(MAAT00.BIZ_RESULT, resultMap);
