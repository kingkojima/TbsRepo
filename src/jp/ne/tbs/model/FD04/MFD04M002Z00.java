...
//	private int fmyDisCnt = 0;
	...
	public void execute() throws Exception {
		...
		//ADOより在宅履歴TBLを取得
		List<MDB02T001Z00> zaHistTbls = _allInOneData.getZaHistTbls();
...
//2020/10/09 Add Start J.Kojima
						//死亡・終了の患者を除くフラグを取得
						boolean delflg = Boolean.valueOf(
							_allInOneData.getAppData().getMsgIn(MAAT00.DEL_FLG));
				...
				if (targetSrt.compareTo(fDateDate) <= 0
					...
					if (subject.contains(title)) {
						...
						for (int i = 0; i < ITM_NME_LIST.length; i++) {
							...
							//項目１　項目名　取得
							String nme1 = _allInOneData.getAppData().getMsgIn(ITM_NME_LIST[i]);
							...
							if (!nme1.equals(MAAT00.CHAR.EMPTY_STRING)
								...
								//項目１　区切り文字　取得
								String dlm1 = _allInOneData.getAppData().getMsgIn(ITM_DLM_LIST[i]);
								//項目１　終了文字　取得
								String end1 = _allInOneData.getAppData().getMsgIn(ITM_END_LIST[i]);
								//終了文字のインデックスを定義
								int indexEnd1 = 0;
...
//								}
	...
	public void setAllInOneData(MAA00B002Z00 allInOneData) {
		this._allInOneData = allInOneData;
	}
